...
public class ShakeEventManager implements SensorEventListener {
		...
		if (maxAcc >= MOV_THRESHOLD) {
			if (counter == 0) {
				...
				if ((System.currentTimeMillis() - firstMovTime) < SHAKE_WINDOW_TIME_INTERVAL) {
					...
					Log.d("xOS", "Second move outside time interval: " + (System.currentTimeMillis() - firstMovTime));
					counter++;
					firstMovTime = System.currentTimeMillis();
					return;
				}
	...
	private void showUndoRedoDialog() {
		...
		builder.setTitle("");
		if (canUndo) {
			builder.setPositiveButton(mContext.getResources().getString(com.android.internal.R.string.undo), new OnClickListener() {
				@Override
		...
		if (canRedo) {
			builder.setNegativeButton(mContext.getResources().getString(com.android.internal.R.string.redo), new OnClickListener() {	
				@Override
				public void onClick(DialogInterface dialog, int which) {
					mUndoRedo.redo();
					mDialogShowing = false;
